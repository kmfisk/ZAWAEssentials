...
public class KakarikiRenderer extends ZawaMobRenderer<KakarikiEntity, ParrotBaseModel<KakarikiEntity>> {
    private final ParrotBaseModel<KakarikiEntity> adultModel;
    private final ParrotBaseModel<KakarikiEntity> flyingModel;
    private final ParrotBaseModel<KakarikiEntity> babyModel;
    ...
    @Override
    public void render(KakarikiEntity entity, float entityYaw, float partialTicks, MatrixStack matrixStack, IRenderTypeBuffer buffer, int packedLight) {
    ...
    @Override
    public void setupAdultTextures(KakarikiEntity entity) {
        int variantCount = entity.getTotalVariants();
        adultTextures = new ResourceLocation[variantCount];
        for (int i = 0; i < variantCount; i++)
            adultTextures[i] = new ResourceLocation(ZawaEssentials.MOD_ID, "textures/entity/kakariki/kakariki_" + (i + 1) + ".png");
    }

    @Override
    public void setupBabyTextures(KakarikiEntity entity) {
        int variantCount = entity.getTotalVariants();
        babyTextures = new ResourceLocation[variantCount];
        for (int i = 0; i < variantCount; i++)
            babyTextures[i] = new ResourceLocation(ZawaEssentials.MOD_ID, "textures/entity/kakariki/kakariki_" + (i + 1) + ".png");
    }
}
